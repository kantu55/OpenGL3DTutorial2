...
#define MAKE_FOURCC(a, b, c, d) \
	...
	*/
	struct DDSHeader
	{
		...
		uint32_t reserved[11]; // (将来のために予約されている)
	...
	*/
	GLuint LoadDDS(const char* filename)
	{
		...
		// 1メガバイトのメモリを割り当てて、一度にたくさん読み込めるようにする
	...
	*/
	bool LoadImage2D(const char* path, ImageData* imageData)
	{
		...
		std::vector<uint8_t> tgaBuf(1000);
		ifs.rdbuf()->pubsetbuf(tgaBuf.data(), tgaBuf.size());
		...
		std::vector<uint8_t> imageBuf(imageSize);
		ifs.read(imageBuf.data(), imageSize);
		...
		if (tgaHeader[17] & 0x20)
		{
			...
			std::vector<uint8_t>::iterator source = imageBuf.begin();
