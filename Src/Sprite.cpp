...
void SpriteRenderer::Draw(const glm::vec2& screenSize) const
{
	...
	for (const Primitive& primitives : primitives)
	{
		program->BindTexture(0, primitives.texture->Get());
		glDrawElements(GL_TRIANGLES, primitives.count, GL_UNSIGNED_SHORT,
			reinterpret_cast<const GLvoid*>(primitives.offset));
	}
