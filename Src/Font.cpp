...
@retval false 初期化失敗
*/
bool FontRenderer::Init(size_t maxCharcter)
{
	return spriteRenderer.Init(maxCharcter, "Res/Sprite.vert", "Res/Sprite.frag");
}
...
{
	/*
	std::unique_ptr…「デストラクタで実行する関数」を指定する機能がある
	*/
	//ファイルを開く
	//ファイル操作が終わればfclose関数でデストラクタする
	...
	int ret = fscanf(fp.get(),
		...
		&spacing[0], &spacing[1]);
	/*
	2つのデータを取得しようとしているので戻り値が2未満の場合はエラー
	*/
	if (ret < 2)
	{
...
bool FontRenderer::AddString(const glm::vec2& position, const wchar_t* str)
{
	...
	for (const wchar_t* itr = str ; *itr ; ++itr)
	{
		...
		{
			//スプライトの座標は中心を指定するが、フォントは左上を指定する
